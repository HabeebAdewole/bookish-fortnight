...
export function CoachesPage() {
  return <>
    <section className="coaches-intro page-width"><p>Coaching at FORM</p><div><h1 tabIndex={-1}>GOOD TRAINING.<br />STARTS WITH GUIDANCE.</h1><p>Get to know the approaches behind our sample sessions. Strength, conditioning, and mobility, each with a clear purpose.</p></div></section>
    <section className="coach-directory page-width" aria-label="Coaching team">{trainers.map(coach => <article className="coach-card" key={coach.id}><a className="coach-card-image" href={`#/coaches/${coach.id}`} tabIndex={-1} aria-hidden="true"><img src={`/images/stock/${coach.image}`} alt="" width="800" height="1000" loading="lazy" /><span>{coach.category}</span></a><p className="coach-specialty">{coach.specialty}</p><h2><a href={`#/coaches/${coach.id}`}>{coach.name}</a></h2><p>{coach.description}</p><a className="text-link" href={`#/coaches/${coach.id}`}>Meet {coach.name.split(' ')[0]} <span aria-hidden="true">↗</span></a></article>)}</section>
...
export function CoachProfile({ coach }: { coach: Trainer }) {
  ...
  return <>
    <section className="profile-hero"><div className="profile-heading"><a className="profile-back" href="#/coaches">← All coaches</a><p>{coach.specialty}</p><h1 tabIndex={-1}>{coach.name.split(' ').map(name => <span key={name}>{name}</span>)}</h1><p className="profile-statement">{coach.description}</p><a className="button button-dark" href={`#/classes?coach=${coach.id}`}>View {firstName}’s sessions</a><small>Fictional coach profile. Image illustrates the training discipline.</small></div><img src={`/images/stock/${coach.image}`} alt={coach.imageAlt} width="1000" height="1200" /></section>
